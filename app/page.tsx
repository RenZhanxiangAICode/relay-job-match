...
type ConversationItem = { id:string; matchId:string; anonymousCode:string; score:number; status:"active"|"cancelled"|"success_pending"|"successful"; createdAt:number; perspective:Mode; payload:Record<string,string>; reasons:string[]; risks:string[]; verifyOnMeeting:string[]; lastMessage?:string|null; messageCount:number; reviewedByMe?:boolean };
...
const conversationStatusLabel = (status:ConversationItem["status"]) => status==="active"?"匿名沟通中":status==="success_pending"?"等待合作确认":status==="successful"?"合作成功":"会话已结束";
...
export default function Home() {
  ...
  return <main className="app-shell" onClickCapture={buttonFeedback}>
    ...
    {reportOpen&&<div className="modal-backdrop" onClick={()=>setReportOpen(false)}><form className="action-modal" onSubmit={submitReport} onClick={event=>event.stopPropagation()}><header><div><span className="overline">REPORT WITH EVIDENCE</span><h2>举报这位匿名用户</h2></div><button type="button" onClick={()=>setReportOpen(false)}>×</button></header><p>举报会先脱敏，再随机分配给符合条件的陪审员。请只提交与本次沟通直接相关的证据。</p><label><span>举报理由</span><select value={reportCategory} onChange={event=>setReportCategory(event.target.value)}><option value="false_job">虚假岗位或 HC</option><option value="false_resume">虚假简历或项目经历</option><option value="fraud">索要费用或诈骗</option><option value="harassment">骚扰或不当沟通</option><option value="other">其他</option></select></label><label><span>情况说明</span><textarea rows={5} maxLength={1200} value={reportSummary} onChange={event=>setReportSummary(event.target.value)} placeholder="说明发生了什么、时间线和需要核实的重点…"/></label><label className="evidence-upload"><span>证据截图（1—3 张）</span><input type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={event=>setReportEvidence(Array.from(event.target.files||[]).slice(0,3))}/><small>{reportEvidence.length?`已选择 ${reportEvidence.length} 张；每张不超过 5MB`:`支持 PNG、JPG、WebP，每张不超过 5MB`}</small></label><footer><button type="button" className="outline" onClick={()=>setReportOpen(false)}>取消</button><button className="solid" disabled={busy||!reportSummary.trim()||reportEvidence.length===0}>{busy?"正在提交…":"提交举报"}</button></footer></form></div>}
    {reviewOpen&&activeConversationItem&&<div className="modal-backdrop" onClick={()=>setReviewOpen(false)}><form className="action-modal review-modal" onSubmit={submitReview} onClick={event=>event.stopPropagation()}><header><div><span className="overline">PRIVATE REVIEW</span><h2>评价本次合作</h2></div><button type="button" onClick={()=>setReviewOpen(false)}>×</button></header><p>评价总分 100 分。结果会按信誉规则影响对方，但不会公开你的真实身份。</p>{([['truthfulness','信息真实性',25],['attitude','沟通态度',20],['responsiveness','回复及时性',15],['professionalism','专业程度',20],['fulfillment','履约情况',20]] as const).map(([key,label,max])=><label className="score-field" key={key}><span>{label}</span><input type="range" min="0" max={max} value={reviewValues[key]} onChange={event=>setReviewValues(values=>({...values,[key]:Number(event.target.value)}))}/><b>{reviewValues[key]} / {max}</b></label>)}<label><span>评价说明（选填）</span><textarea rows={4} maxLength={800} value={reviewValues.comment} onChange={event=>setReviewValues(values=>({...values,comment:event.target.value}))} placeholder="可说明做得好的地方，以及后续需要核实或改进的事项…"/></label><div className="review-total"><span>总分</span><b>{reviewValues.truthfulness+reviewValues.attitude+reviewValues.responsiveness+reviewValues.professionalism+reviewValues.fulfillment}</b><small>/ 100</small></div><footer><button type="button" className="outline" onClick={()=>setReviewOpen(false)}>取消</button><button className="solid" disabled={busy}>{busy?"正在提交…":"提交评价"}</button></footer></form></div>}
    {toast&&<div className="toast">{toast}</div>}
  </main>;
}
