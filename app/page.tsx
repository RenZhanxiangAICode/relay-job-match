...
export default function Home() {
  ...
  return <main className="app-shell" onClickCapture={buttonFeedback}>
    ...
    <section className="workspace">
      ...
      {view==="history"&&<div className="page history-page"><div className="page-heading"><div><span className="overline">SUCCESSFUL MATCH HISTORY</span><h1>历史匹配</h1><p>这里只保留双方已经成功配对并建立匿名会话的历史记录。</p></div></div>{historyItems.length===0?<div className="empty-state"><span>◷</span><h2>还没有成功匹配记录</h2><p>双方都点击“想了解”并成功配对后，过往记录会保留在这里。</p></div>:<div className="history-list">{historyItems.map(item=><article key={item.id}><div><span>{item.weekKey}</span><h2>{item.perspective==="talent"?"匿名岗位":"匿名候选人"} {item.anonymousCode}</h2><p>成功配对 · 匹配度 {item.score} 分</p></div><i className="success">成功</i>{item.reviewAvailable&&<button className="outline" onClick={()=>flash("评价与追评将在评价功能上线后开放")}>评价 / 追评</button>}</article>)}</div>}</div>}
