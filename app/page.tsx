...
export default function Home() {
  ...
  const refreshDashboard=useCallback(async()=>{
    ...
    try{
      ...
      const data = await response.json() as {user:{email:string;isAdmin:boolean};profiles:Array<{type:Mode;anonymousCode:string;status:string;completion:number;payload:Record<string,string>}>;readyForMatching:boolean;matches:MatchItem[];notifications:NotificationItem[];conversations:ConversationItem[]};
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      <header className="topbar"><div><span className="secure-dot"/>匿名模式已开启</div><div><button onClick={()=>nav("notifications")}>○ 通知{notifications.length>0?` ${notifications.length}`:""}</button><button className="score-pill" onClick={()=>nav("trust")}>信誉 80</button></div></header>
      ...
      {view==="posts"&&<div className="page posts-page"><div className="page-heading"><div><span className="overline">MY PRIVATE POSTS</span><h1>我的发布</h1><p>每个账号只能同时向池子提交一条接棒信息和一条找工作信息，可随时修改。</p></div><div className="post-limit"><b>{Object.values(profileMeta).filter(Boolean).length} / 2</b><span>已使用发布名额</span></div></div><div className="post-cards">{(["role","talent"] as Mode[]).map(item=>{const profile=profileMeta[item];return profile?<article key={item}><header><span>{profile.anonymousCode}</span><i>匹配中</i></header><h2>{item==="role"?"我的待接棒岗位":"我的找工作画像"}</h2><p>{item==="role"?[profile.payload.city,profile.payload.role,profile.payload.industry].filter(Boolean).join(" · "):[profile.payload.industry,profile.payload.city,profile.payload.salary].filter(Boolean).join(" · ")}</p><div className="post-meta"><span>画像完整度 <b>{profile.completion}%</b></span><span>本周匹配 <b>{matchItems.filter(match=>match.perspective===item).length}</b></span></div><div><button className="solid" onClick={()=>{setMode(item);setProfileOpen(true)}}>{item==="role"?"修改接棒信息":"修改求职信息"}</button></div></article>:<article className="empty-post" key={item}><span className="card-index">{item==="role"?"ROLE POST":"TALENT POST"}</span><h2>{item==="role"?"还没有待接棒岗位":"还没有求职画像"}</h2><p>{item==="role"?"提交真实岗位信息后，才会进入候选人匹配池。":"提交能力与求职偏好后，才会进入岗位匹配池。"}</p><button className="solid" onClick={()=>{setMode(item);setProfileOpen(true)}}>立即发布</button></article>})}</div><div className="one-post-rule"><b>为什么限制为 1+1？</b><p>减少重复岗位、批量投放和虚假广告。如果方向发生变化，请直接修改原信息。</p></div></div>}
