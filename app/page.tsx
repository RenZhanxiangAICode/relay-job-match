...
const matches = [
  { score:96, title:"匿名岗位 R-1904", city:"上海·混合", salary:"32–40K", verified:"发布者手机已认证", why:"你的 B 端产品经验、复杂需求拆解和跨部门推进能力完全命中。", risk:"公司、任职、HC 和业绩均为发布者自述；需在匿名沟通中逐项验证。" },
  { score:93, title:"匿名岗位 R-2088", city:"杭州", salary:"35–45K", verified:"发布者手机已认证", why:"你过去的用户增长经验与对方描述的业务阶段高度一致。", risk:"岗位真实性、团队规模和 20% 出差频率均尚未由第三方认证。" },
  { score:91, title:"匿名岗位 R-2261", city:"北京·可远程", salary:"30–38K", verified:"发布者手机已认证", why:"你在广告策略与产品协作上的复合经验与对方需求类似。", risk:"内推权限、薪资口径与远程政策都属于自述信息。" },
];
...
export default function Home() {
  ...
  if(!loggedIn) return <main className="login-page">
    ...
    <section className="login-copy"><span className="overline">PRIVATE TALENT NETWORK</span><h1>不用浏览。<br/><em>只见真正适合的人。</em></h1><p>你的岗位和简历都不会公开。AI 每周从私密池中筛选 10 个机会，只在匹配超过 90 分时为双方建立连接。</p><div className="login-proof"><span>仅手机号认证</span><span>全程匿名沟通</span><span>社区过半自动处置</span></div></section>
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {view==="home"&&<div className="page home-page">
        ...
        <section className="principles"><div><span>90+</span><p>只有超过 90 分才建立连接</p></div><div><span>10</span><p>每周最多十个，不制造无限滑动</p></div><div><span>●</span><p>第一版仅认证手机，其他信息都是自述</p></div><div><span>50%</span><p>陪审拉黑票过半后，系统立即自动封号</p></div></section>
      </div>}
      ...
      {view==="messages"&&<div className="page messages-page"><div className="page-heading"><div><span className="overline">ANONYMOUS CONNECTIONS</span><h1>匿名沟通</h1><p>双方全匿名，第一版仅显示“手机已认证”，任职、学历、HC 与业绩均不做平台认证。</p></div></div><div className="messenger"><aside><button className="chat-person active"><span>T</span><div><b>匿名候选人 T-8821</b><p>刚刚：我想了解实际工作负荷…</p></div><i>2</i></button><button className="chat-person"><span>R</span><div><b>匿名岗位 R-1904</b><p>昨天：可以先聊一下你的期望</p></div></button></aside><section className="chat-panel"><header><div><b>匿名候选人 T-8821</b><span>94 分匹配 · 手机已认证</span></div><i className="anonymous-badge">不解锁真实身份</i></header><div className="privacy-banner">◈ 手机、微信、邮箱和真实姓名不会对匹配对象展示。</div><div className="conversation"><div className="bubble them"><span>T</span><p>你好，AI 提醒我这个岗位的管理风格可能偏强。想了解一下，所谓“强”是要求细致，还是会频繁改变方向？</p></div><div className="bubble me"><p>更接近决策快、对结果要求高。方向不会频繁变，但遇到客户问题时需要很快响应。</p></div><div className="bubble them"><span>T</span><p>了解，这点我可以接受。你能说一下前三个月最重要的交付吗？</p></div></div><form className="chat-compose" onSubmit={e=>{e.preventDefault();flash("演示消息已发送")}}><input placeholder="在不暴露身份的前提下回复…"/><button>发送</button></form></section></div></div>}

      {view==="trust"&&<div className="page trust-page"><div className="page-heading"><div><span className="overline">TRUST PASSPORT</span><h1>信誉记录</h1><p>第一版只认证手机号归属，其他岗位与简历信息都是本人自述。</p></div><div className="trust-score"><b>92</b><span>良好信誉</span></div></div><div className="trust-layout"><section className="passport"><header><div className="avatar">R</div><div><h2>匿名用户 2716</h2><p>手机已认证 · 加入 38 天</p></div><span className="level">手机认证</span></header>{[["手机号归属","verified","短信验证已通过"],["任职与学历","self","平台暂不认证"],["项目成果","self","平台暂不认证"],["专业能力","self","平台暂不认证"],["内推与 HC 权限","self","平台暂不认证"],["社区履约记录","record","12 次沟通·0 次有效投诉"]].map(x=><div className="passport-row" key={x[0]}><div><b>{x[0]}</b><small>{x[2]}</small></div><span className={x[1]}>{x[1]==="verified"?"已认证":x[1]==="record"?"平台记录":"本人自述"}</span></div>)}</section><aside className="reputation"><h3>声誉如何影响匹配</h3><p>真实沟通、良好履约与负责任的陪审会提高权重；造假、骗钱与多次恶意投票会扣减信誉。</p><div className="rep-item"><span>+5</span><div><b>匹配后好评</b><small>4 位匹配对象评价“沟通真诚”</small></div></div><div className="rep-item"><span>+2</span><div><b>负责任陪审</b><small>已完成 6 次有效陪审</small></div></div><div className="rep-item negative"><span>-12</span><div><b>恶意陪审惩罚</b><small>申诉复核确认恶意投票时扣减</small></div></div><button onClick={()=>flash("信誉明细已展示，所有扣分均可申诉")}>查看完整信誉规则</button></aside></div></div>}

      {view==="jury"&&<div className="page jury-page"><div className="page-heading"><div><span className="overline">COMMUNITY JURY</span><h1>公民陪审</h1><p>随机抽取高信誉用户投票；拉黑票超过 50% 时系统立即封号，不等待人工复核。</p></div><div className="jury-duty"><b>当前封号线：过半票</b><span>被封用户可提交一次人工申诉</span></div></div><div className="jury-layout"><aside>{juryCases.map((c,i)=><button className={selectedCase===i?"active":""} onClick={()=>{setSelectedCase(i);setVoted(null)}} key={c.id}><span>{c.id}</span><b>{c.type}</b><small>{c.votes} 票已提交</small></button>)}</aside><section className="case-file"><header><div><span>匿名案件 {juryCases[selectedCase].id}</span><h2>{juryCases[selectedCase].type}</h2></div><i>50%+ 自动封号</i></header><div className="case-warning">你看不到任何姓名、公司全称或联系方式。申诉如证明陪审者多次恶意投票，其信誉分将被扣减。</div><div className="vote-meter"><div><b>当前拉黑票 46%</b><span>距离自动封号还差 5%</span></div><i><b/></i></div><h3>争议摘要</h3><p>{juryCases[selectedCase].summary}</p><h3>脱敏证据</h3><div className="evidence"><span>▣</span><div><b>{juryCases[selectedCase].evidence}</b><small>证据已脱敏·陪审者自行判断</small></div><button>查看</button></div><h3>你的投票</h3><div className="verdicts"><button className={voted==="black"?"selected danger":""} onClick={()=>setVoted("black")}><b>拉黑账号</b><span>这一票可能触发系统立即封号</span></button><button className={voted==="keep"?"selected":""} onClick={()=>setVoted("keep")}><b>不应拉黑</b><span>证据不足或行为未达到封号程度</span></button></div><button className="submit-verdict" disabled={!voted} onClick={()=>flash(voted==="black"?"投票后拉黑票已过半，系统自动封号":"匿名陪审票已提交")}>提交匿名投票</button><p className="appeal-note">封号后可在 7 天内申诉。人工只处理申诉与恶意陪审调查，不介入首次投票结果。</p></section></div></div>}

      {view==="admin"&&<div className="page admin-page"><div className="page-heading"><div><span className="overline">APPEAL & JURY QUALITY</span><h1>申诉与陪审质量</h1><p>首次处置完全由陪审票决定；人工只复核被封账号的申诉，并调查是否存在多次恶意投票。</p></div><button className="export-btn" onClick={()=>flash("已生成脱敏陪审质量报告")}>导出陪审报告</button></div><div className="admin-stats">{[["8","待处理申诉"],["3","疑似恶意陪审者"],["12","今日过半自动封号"],["4.1%","封号申诉率"]].map(x=><div key={x[1]}><b>{x[0]}</b><span>{x[1]}</span></div>)}</div><section className="risk-table"><header><h2>申诉复核队列</h2><div><button className="active">全部</button><button>虚假广告</button><button>涉嫌骗钱</button><button>简历造假</button></div></header><div className="table-head"><span>匿名账号 / 申诉</span><span>原封号原因</span><span>陪审结果</span><span>状态</span><span>操作</span></div>{[["R-88102 · 已提交申诉","索要内推费","57% 拉黑票","待申诉复核"],["T-09211 · 已提交申诉","任职时间矛盾","64% 拉黑票","调查陪审者"],["R-12094 · 申诉已完成","批量重复岗位","52% 拉黑票","维持封号"]].map((x,i)=><div className="table-row" key={x[0]}><span><b>{x[0]}</b><small>仅手机已认证</small></span><span>{x[1]}</span><span>{x[2]}</span><span><i className={i===0?"pending":i===1?"danger":"limited"}>{x[3]}</i></span><span className="row-actions"><button onClick={()=>flash(i===1?"已对涉嫌多次恶意投票者扣减信誉分":"申诉复核结果已记录")}>{i===1?"扣陪审信誉":"处理申诉"}</button></span></div>)}</section></div>}
    </section>
