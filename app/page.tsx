  ...
  return <main className="app-shell" onClickCapture={buttonFeedback}>
    ...
    <section className="workspace">
      ...
      {view==="home"&&<div className="page home-page">
        <section className="welcome"><div><span className="overline">YOUR PRIVATE CAREER STATUS</span><h1>{hasUnreadMessage?"有人在等你的回复。":matchItems.length?`今天有 ${matchItems.length} 个机会，值得认真判断。`:draftProfile?"把真实经历说完整，下一棒才会更准。":"你的下一棒，正在私密池中寻找你。"}</h1><p>{hasUnreadMessage?"回复从真实交流开始；连续短句不会被当成有效沟通。":matchItems.length?"系统已经整理好匹配原因、风险和需要验证的事项。":readyForMatching?"画像已进入私密池，目前只等待真实用户出现。":"先选择一个目标，AI 会用访谈帮你建立画像。"}</p></div><div className="welcome-side"><aside className="daily-tip"><span>今日求职 TIP · {jobDirection}</span><p>{dailyCareerTip}</p></aside><div className="cycle"><span>距离下一次每日更新</span><b>{nextDailyCountdown.hours}<small>时</small> {nextDailyCountdown.minutes}<small>分</small></b><p>每天 00:00 更新 · 两个方向各最多 10 条</p></div></div></section>
