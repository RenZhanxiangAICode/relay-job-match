...
邮箱、账号、发布、月度额度和匹配结果均接入真实服务与数据库。AI 解析使用服务端 `OPENAI_API_KEY`，默认模型可通过 `OPENAI_MODEL` 调整。当前匹配向量层使用本地确定性 128 维文本向量；后续可替换为语义 embedding，并沿用相同的数据库候选召回和匹配记录结构。
